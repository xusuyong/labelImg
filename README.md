...
命令行启动时可指定类别文件：
```bash
nineskylabelimg <图片目录> <类别文件路径> [保存目录]
```

**类别文件优先级说明：**

- **命令行参数指定的类别文件**：用于预定义标签列表，在新建标注时显示在标签选择框中
- **图片目录中的 classes.txt**：当加载已有的 YOLO 标注文件（.txt）时，程序会自动从该文件所在目录查找 `classes.txt` 来解析类别名称。

这是因为每个标注目录可能有不同的类别定义，加载已有标注时使用目录中的类别文件可以确保正确解析标签。
